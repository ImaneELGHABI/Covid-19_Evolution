...
#Renaming some columns using the following command.
...
#<m.choropleth> allows us to draw our colored map

m.choropleth(geo_data=json, data=covid_reg, # First, we specify the data
columns=['ID', 'Active_cases'], # The following columns belong to our 'data' dataframe.
key_on='properties.ID', # This column belongs to our 'geo_data' json file, it contains the same data as the 'ID' column.
# These are the common columns between the two datasets, which allowed us to link them together.
color="Active_cases",
projection="mercator",
...
fields=['Name-FR','Active cases'],
aliases=['Region: ','Active Cases: '],
style=("background-color: white; color: #333333; font-family: arial; font-size: 12px; padding: 10px;") )
)
m.add_child(sty)
m.keep_in_front(sty)
folium.LayerControl().add_to(m)
#With this last line, we saved our map to an existing html file.
